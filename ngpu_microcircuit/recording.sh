#!/bin/bash
# Recording script for executing 10 simulations
...
for seed in {0..9}; do
	...
	echo "Recording: seed $seed, id $sim_id"
	...
	# Run locally
	python3 run_recording.py recording_times_$run_id --path=$run_path --seed=$seed --algo=$nl_algo 2> $data_path/run_recording_$run_id.err 1> $data_path/run_recording_$run_id.out

	# Run with slurm
	# srun --ntasks-per-node=$procs --cpus-per-task=$threads --threads-per-core=1 --cpu-bind=verbose,rank --error=$data_path/run_recording_$run_id.err --output=$data_path/run_recording_$run_id.out python3 run_recording.py recording_times_$run_id --path=$run_path --seed=$seed --algo=$nl_algo

done

python3 gather_data.py $data_path --out=$data_path/recording_data_$sim_id.json
