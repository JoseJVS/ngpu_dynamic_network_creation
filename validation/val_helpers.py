...
def __get_distributions(set = ['Sim 1', 'Sim 2', 'Sim 3']):
    """ Gets distributions of firing rate, CV ISI and Pearson correlation.

    Parameters
    ----------
    set
        list of strings specifying which sets of simulation toy want to get the distributions from

    Returns
    -------
    Values of the computed statistic measure for each neuron of the population. The values are saved
    in .dat files inside the dataX directories produced with the simulations.
...
def __get_distributions_csv(simulation = 'Sim 1'):
    """ Gets csv files of distributions results from the .dat files obtained by __get_distributions function.

    Parameters
    ----------
    simulation
            string (can be "Sim 1" or "Sim 2") needed to choose the set of simulations to be placed side-by-side with "Set 3"
...
def __plot_distributions(run_id=0):
    ...
    ----------
    run_id
        id of the simulation to be plotted. For example, if run_id=0, the distributions got from
        the results of the simulation labelled as data0 of Sim 1-2 and Sim 3 are plotted.
...
def __get_emd():
    ...
    Returns
    -------
    .dat files containing the values of the EMD comparison for each population and each pair of simulations.
    In particular, the comparisons "Sim 1 - Sim 2" and "Sim 1 - Sim 3" are computed.
...
def __get_emd_csv():
    """ Gets csv files containing the EMD pairwise comparison.

    Returns
    -------
    A csv file for each distribution computed. These files are needed to produce the
    EMD box plots.
