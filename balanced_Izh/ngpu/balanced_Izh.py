...
exc_neuron = ngpu.Create("izhikevich", NE, n_receptors)
inh_neuron = ngpu.Create("izhikevich", NI, n_receptors)

ngpu.SetStatus(exc_neuron, params["exc_params"])
ngpu.SetStatus(inh_neuron, params["inh_params"])
...
exc_syn_dict={"weight": Wex, "delay": {"distribution": "normal_clipped", "high": params["delay_max"], "low": params["delay_min"], "mu": params["delay_mu"], "sigma": params["delay_sigma"]}}
...
inh_syn_dict={"weight": Win, "delay": {"distribution": "normal_clipped", "high": params["delay_max"], "low": params["delay_min"], "mu": params["delay_mu"], "sigma": params["delay_sigma"]}}
...
if recording:
    ...
    for i in range(len(exc_spike_times)):
        frE[i] = len(exc_spike_times[i])/(sim_time/1000.0)
    for i in range(len(inh_spike_times)):
        frI[i] = len(inh_spike_times[i])/(sim_time/1000.0)
