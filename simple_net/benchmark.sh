...
for conn in all_to_all one_to_one; do
    for neur in 1000 10000; do
        for seed in {0..2}; do
            seed=12345$seed

            # Simulation identifier
            run_id=$neur\_$conn\_$seed\_$sim_id
            echo "Benchmark: neurons $neur seed $seed, id $sim_id"

            # For each seed a new directory is created
            run_path=$data_path/$conn/$neur\_neurons/seed_$seed\_id_$sim_id
            ...
            # Run locally
            python3 simple_net.py benchmark_times_$run_id --neurons=$neur --connectivity=$conn --path=$run_path --seed=$seed --algo=$nl_algo 2> $data_path/run_benchmark_$run_id.err 1> $data_path/run_benchmark_$run_id.out
        ...
        python3 gather_data.py $data_path/$conn/$neur\_neurons --out=$data_path/benchmark\_data\_$neur\_$conn\_$sim_id.json
    done
done
